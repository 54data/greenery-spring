<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<div class="mypage-myinfo-title">개인정보 수정</div>
<div class="myinfo-list">
	<div class="myinfo-id">
		<span>아이디</span> <span>honggildong</span>
	</div>
	<div class="myinfo-name">
		<span>이름</span> <span>홍길동</span>
	</div>
	<div class="myinfo-phone">
		<span>전화번호</span> <span>010-0000-0000</span>
	</div>
	<div class="myinfo-address">
		<span>주소</span> <span>서울특별시 종로구 명륜2가</span>
	</div>
	<div class="myinfo-email">
		<span>이메일</span> <span>honggildong@mail.com</span>
	</div>
	<div class="myinfo-edit">
		<button class="myinfo-edit-btn">개인정보 수정</button>
	</div>
</div>